...
function AchievementCard({
  ...
  return (
    <Animated.View
      style={[
        styles.achCard,
        a.unlocked && styles.achCardUnlocked,
        isNew && styles.achCardNew,
...
export default function ComunidadScreen({
  ...
  const { rankingByMonster, rankingByUser, totalCommunityLatas, loading, error, refresh } =
    useGlobalStats();
  const {
    requests,
  ...
  useEffect(() => {
    if (!loading && !requestsLoading && refreshing) {
      setRefreshing(false);
    }
  ...
  return (
    ...
    >
        ...
        (() => {
          ...
          const pieData = rankingByMonster.map((entry) => {
            ...
            return {
              ...
              text: pct >= 5 ? `${pct}%` : '',
              name: getMonsterName(entry.monsterId)
                .replace('Monster ', '')
                .replace('Juice ', '')
                .split(' ')
                .slice(0, 2)
          ...
          const handleDelete = () => {
            ...
            Alert.alert(
              ...
              [
                ...
                {
                  text: t('comunidad.requestDelete'),
                  style: 'destructive',
            ...
            <View key={req.id} style={styles.requestCard}>
              ...
              <View style={styles.requestInfo}>
                ...
                {req.authorName && (
                  <Text style={styles.requestAuthor}>
                    {t('comunidad.requestBy', { name: req.authorName })}
                  </Text>
                )}
              </View>
              <View style={styles.requestActions}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.requestVoteCount, req.hasVoted && styles.requestVoteCountActive]}
                  >
                    {req.voteCount}
                  </Text>
                </TouchableOpacity>
      ...
      >
        <Pressable style={styles.imageViewerOverlay} onPress={() => setImageViewerUri(null)}>
          {imageViewerUri && (
            <Image
